...
def filter_by_length(conf, signals, geodesics, skels, skel_ids):
    mask = [
        False if g[-1] - g[0] >= conf.filter_all.thres_length else True
        for g in geodesics
    ]
    ...
    dists = list(
        map(
            lambda x: np.linalg.norm(
                np.cross(x - p0, x - p1) / np.linalg.norm(p1 - p0)
            ),
            skel.vertices,
        )
    )
...
def filter_by_shape(conf, signals, geodesics, skels, skel_ids):
    ...
    mask_mean = [
        False if m < l * conf.filter_all.thres_shape_mean else True
        for m, l in zip(np.mean(dists, axis=1), lens)
    ]
    mask_max = [
...
def filter_by_cell(conf, signals, geodesics, skels, skel_ids, fiber_seg, cell_seg):
    ...
    allow_list = np.array(
        [idx for idx, count in zip(allow_list, counts) if count > 300]
    )
...
def filter_by_ratio(conf, signals, geodesics, skels, skel_ids):
    bbox_dims = list(
        map(
            lambda skel: np.max(skel.vertices, axis=0) - np.min(skel.vertices, axis=0),
            skels,
        )
    )
    with Pool(conf.num_cpus) as p:
        mask = [
            False if cubeness < conf.filter_all.thres_ratio else True
            for cubeness in list(p.map(cubeness_metric, bbox_dims))
        ]
    signals = np.delete(signals, mask, axis=1)
    geodesics = np.delete(geodesics, mask, axis=0)
    skels = np.delete(skels, mask, axis=0)
...
if __name__ == "__main__":
    ...
    for basename in (pbar := tqdm(basenames)):
        ...
        skels = np.load(
            os.path.join(output_path, basename + "-fiber_skel.npz"), allow_pickle=True
        )
        ...
        prior = len(geodesics)
        signals, geodesics, skels, skel_ids = filter_by_length(
            conf, signals, geodesics, skels, skel_ids
        )
        signals, geodesics, skels, skel_ids = filter_by_shape(
            conf, signals, geodesics, skels, skel_ids
        )
        signals, geodesics, skels, skel_ids = filter_by_cell(
            conf, signals, geodesics, skels, skel_ids, fiber_seg, cell_seg
        )
        signals, geodesics, skels, skel_ids = filter_by_ratio(
            conf, signals, geodesics, skels, skel_ids
        )
        post = len(geodesics)
